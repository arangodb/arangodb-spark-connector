...
class ArangoRDD[T: ClassTag](
  ...
  private def createCursor(arangoDB: ArangoDB, readOptions: ReadOptions, partition: ArangoPartition)(implicit clazz: ClassTag[T]): ArangoCursor[T] =
    arangoDB.db(readOptions.database).query(s"FOR doc IN @@col ${createFilter()} RETURN doc", partition.bindVars.asJava,
      partition.queryOptions.stream(true), clazz.runtimeClass.asInstanceOf[Class[T]])
